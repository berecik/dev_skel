...
MANIFEST = {
    ...
    "notes": (
        ...
        "confirm the new {item_class} layer compiles. After generation, "
        "manually wire the new {item_class}sController + {item_class}Form + "
        "{item_class}ListView into `lib/screens/home_screen.dart` "
        "(home_screen.dart is NOT regenerated to avoid type mismatches "
        "with main.dart's fixed controller parameter types). The "
        "wrapper-shared `<wrapper>/.env` is baked in via "
...
""",
        ...
        # main.dart passes `itemsController: ItemsController` — if the AI
        # rewrites home_screen.dart to use `TicketsController`, the type
        # mismatch causes a compile error that the model cannot self-fix.
        # The user wires the new controller into home_screen.dart manually
        # after reviewing the generated form + list widgets.
    ],
}
