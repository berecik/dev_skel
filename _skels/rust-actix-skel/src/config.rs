...
//! out of `web::Data` instead of repeatedly probing `std::env`.
...
pub fn normalize_sqlite_url(raw: &str) -> String {
    ...
    // Honour ":memory:" verbatim so test setups can use an in-memory DB.
    if path_part == ":memory:" {
        return "sqlite::memory:".to_string();
    }

    // Strip query string before path resolution; we re-add it with our
    // own `mode=rwc` to make sqlite auto-create the file.
    ...
    let resolved: PathBuf = if candidate.is_absolute() {
        ...
        // Fall back to cwd-relative — the binary is being run outside a
        // dev_skel wrapper, so we honour the literal path.
        candidate.to_path_buf()
    };
...
pub fn load_dotenv() {
    ...
    // Local .env always wins because it is loaded second and dotenvy does
    // not overwrite existing values by default — so we use `from_path_override`
    // for the local file.
...
mod tests {
    ...
    fn normalize_makes_relative_sqlite_absolute() {
        ...
        assert!(
            s.starts_with("sqlite:///") || s.starts_with("sqlite://"),
            "got: {s}",
        );
        assert!(s.ends_with("?mode=rwc"));
        // The resolved path must be absolute (start with `/`).
        let path_part = s
            .strip_prefix("sqlite://")
            .unwrap()
            .split('?')
