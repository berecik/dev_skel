...
- Lives at `_skels/python-fastapi-skel/`.
...
3. Ensure generated projects are production-capable but simple to understand.

---

## 2. Files to Check First

When working on `python-fastapi-skel`, always inspect these files first:

1. Skeleton documentation: `_docs/python-fastapi-skel.md`
2. Skeleton Makefile: `_skels/python-fastapi-skel/Makefile`
3. Generator scripts:
   - `_skels/python-fastapi-skel/gen`
   - `_skels/python-fastapi-skel/merge`
   - `_skels/python-fastapi-skel/test_skel`
4. Dependency installers:
   - `_skels/python-fastapi-skel/deps`
   - `_skels/python-fastapi-skel/install-deps`
5. Core application code (may evolve over time, but typically under
   `_skels/python-fastapi-skel/app/` or `core/`).
...
1. Document the reason in `_docs/python-fastapi-skel.md` ("Known
   limitations" or similar section).
2. Optionally add a note in this `AGENTS.md` file explaining why the
   skeleton is pinned.
...
If you change behaviour that is specific to `python-fastapi-skel`, also run:
...
`_docs/python-fastapi-skel.md` and, if relevant, in this file.
