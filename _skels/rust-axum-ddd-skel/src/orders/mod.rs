//! Orders resource — `/api/orders`. Aggregate (multi-table) workflow:
//! each state-mutating operation works on an `Order` plus its
//! `OrderLine` + `OrderAddress` children, and creating a line reaches
//! across to the `catalog` repository to snapshot the unit price.
//!
