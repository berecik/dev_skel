...
//! repository trait (`items::ItemRepository`, …) instead of using
//! this generic verbatim — it's documented here as a convention
//! rather than enforced. Resources that need transactional cross-
//! aggregate writes (orders today) declare a per-resource
//! `XxxUnitOfWork` trait whose shape mirrors `AbstractUnitOfWork`
//! below.
...
///
/// Concrete adapters can implement this when their CRUD shape
/// matches; resources with richer needs declare their own resource-
/// specific trait that extends or replaces it entirely.
#[allow(dead_code)]
#[async_trait]
